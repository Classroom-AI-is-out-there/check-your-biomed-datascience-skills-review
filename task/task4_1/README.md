# Task 4.1: classification biomedical modeling
...
| Item | Required value |
|---|---|
| Training data | `dataset/task4_1/train.txt` |
| Test-feature data | `dataset/task4_1/test_features.txt` |
| Local output | `outputs/task4_1_predictions.csv` |
| Exact columns | `sample_id`, `prediction` (no additional columns) |
| Rows | exactly 199, equal to the test examples |

- `sample_id` identifies the test example. Preserve its source data type (values are `str`) and include every test ID exactly once.
- `prediction` is the model result: one of `['ACB', 'ASW', 'ESN', 'GWD', 'LWK', 'MSL', 'YRI']`.
...
submission = pd.DataFrame(
    {
        "sample_id": test_ids,  # extract these from the test-feature data
        "prediction": predicted_labels,
    }
)
output_path = get_output_path("task4_1")
